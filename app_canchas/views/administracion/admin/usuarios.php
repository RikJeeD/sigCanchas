...
<script>
    let frm = $('#gestionUsuarios');
    frm.on("click", ".btn-primary", function () {
        let id = $(this).data('id');
        $.get("/views/administracion/admin/modalUsuario.php?id=" + id, function (data) {
            $("#editUsuario").find("#contenedor").html(data);
        });
    });
    frm.on("click", ".btn-danger", function () {
        let id = $(this).data('id');
