...
<form id="crearModificarUsuario" name="crearModificarUsuario" method="post" action="/Controllers/usuarioController.php"
    ...
    <div class="modal-body">
        ...
        <div id="listadoEmpresas"
             class="form-group has-feedback" <?= (int)$usuario->id_rol !== 2 ? "style=\"display: none;\"" : "" ?>>
            <label for="id_empresa" class="cols-sm-2 control-label">Empresa</label>
            <div class="cols-sm-10">
                <select name="id_empresa" id="id_empresa" class="form-control">
                    ...
                    for ($i = 0; $i < count($empresas); $i++) {
                        ?>
                        <option value="<?= $empresas[$i]->id ?>" <?= $usuario->id_empresa === $empresas[$i]->id ? "selected='true'" : "" ?>><?= $empresas[$i]->nombre ?></option>
                    <?php } ?>
                </select>
            </div>
            <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
...
<script>
    $('#id_rol').change(function () {
        ...
        if (value === 2) {
            ...
            $('#listadoEmpresas').hide();
        }
    });
</script>
<script>
    let frm2 = $('#crearModificarUsuario');
